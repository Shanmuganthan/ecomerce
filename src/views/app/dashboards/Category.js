...
import { getData, patchData, postData, putData } from 'services/Category';
import CategoryForm from 'utility/CategoryForm';
...
const schema = Yup.object().shape({
  ...
  .nullable().required('Product Category is required'),
});
...
const Category = ({ match }) => {
  ...
  const onSubmit = async (values , {resetForm}) => {
    ...
    NotificationManager.success('New Category has been added!','Success!')
    }else{
      const {id} = selectedItem;
      await putData(id,value)
    NotificationManager.success('Category has been updated successfuly!','Success!')
    }
    get()
    toggleModal();
   ...
   const deleteProduct = async() => {
    ...
    NotificationManager.success('Category has been deleted successfuly!','Success!')
    get();
    toggleAlert('');
   }
    ...
    <>
                  ...
                  <table className=" table ">
                    <thead>
                      <tr>
                        ...
                        <th className={`sort-column ${tableOptions.sort === 'name' ? sort[tableOptions.direction] :'' }`} onClick={()=>sorting('name')}>Category Name</th>
                        <th className={`sort-column ${tableOptions.sort === 'parent.name' ? sort[tableOptions.direction] :'' }`} onClick={()=>sorting('parent.name')}>Product Category Name</th>
      ...
      >
        {({ errors , setFieldValue ,values ,touched,setFieldTouched}) => (
          ...
          >
            <CategoryForm setFieldValue ={setFieldValue} setFieldTouched={setFieldTouched} operation={operations}  name='image' errors={errors}  values = {values } touched={touched} />
          </CustomModal>
        )}
      </Formik>
    </>
