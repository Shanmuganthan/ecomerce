...
import { getData, patchData, postData, putData } from 'services/Brands';
import BrandForm from 'utility/BrandForm';
...
const Brands = ({ match }) => {
  ...
  const onSubmit = async (values , {resetForm}) => {
    ...
    NotificationManager.success('New Brand has been added!','Success!')
    }else{
      const {id} = selectedItem;
      await putData(id,values)
    NotificationManager.success('Brand has been updated successfuly!','Success!')
    }
    get()
    toggleModal();
   ...
   const deleteProduct = async() => {
    ...
    NotificationManager.success('Brand has been deleted successfuly!','Success!')
    get();
    toggleAlert('');
   }
...
const gotoPage = (p) => {
  ...
  return (
    <>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.brand" match={match} />
                  ...
                  <table className=" table ">
                    <thead>
                      <tr>
                        <th>S.No</th>
                        <th>Brand Image</th>
                        <th className={`sort-column ${tableOptions.sort === 'name' ? sort[tableOptions.direction] :'' }`} onClick={()=>sorting('name')}>Brand Name</th>
                        <th>Status</th>
                        ...
                        data.map((item, index) => (
                          <tr key={item.name}>
                            ...
                            <td>
                              <img
                                className="table-img"
                                src={`${IMAGE_URL}/brands/${item.image}`}
                                alt={item.name}
                              />
                            </td>
                            <td>{item.name}</td>
      ...
      >
        {({ errors , setFieldValue ,values ,touched}) => (
          ...
          >
            <BrandForm setFieldValue ={setFieldValue} operation={operations}  name='image' errors={errors}  values = {values } touched={touched} />
          </CustomModal>
        )}
      </Formik>
    </>
