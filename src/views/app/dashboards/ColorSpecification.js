...
import { getData, patchData, postData, putData } from 'services/ColorSpecification';
import ColorSpecificationForm from 'utility/ColorSpecificationForm';
...
const title= {
  'A' : 'Add Color SPecifiction',
  'E' : 'Edit Color SPecifiction',
  'V' : 'View Color SPecifiction',
}
const sort = {
  1 : 'sort-asc',
  "-1" : 'sort-desc',
...
const ColorSpecification = ({ match }) => {
  ...
  const onSubmit = async (values , {resetForm}) => {
    ...
    NotificationManager.success('Color Technical Specifiction has been added!','Success!')
    }else{
      const {id} = selectedItem;
      await putData(id,values)
    NotificationManager.success('Color Specifiction has been updated successfuly!','Success!')
    }
    get()
    toggleModal();
   ...
   const deleteProduct = async() => {
    ...
    NotificationManager.success('Color Specifiction has been deleted successfuly!','Success!')
    get();
    toggleAlert('');
   }
...
const gotoPage = (p) => {
  ...
  return (
    <>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="menu.color-specification" match={match} />
      ...
      >
        {({ errors , setFieldValue ,values ,touched}) => (
          ...
          >
            <ColorSpecificationForm setFieldValue ={setFieldValue} operation={operations}  name='image' errors={errors}  values = {values } touched={touched} />
          </CustomModal>
        )}
      </Formik>
    </>
