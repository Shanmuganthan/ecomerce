...
import { getData, patchData, postData, putData } from 'services/SubCategory';
import SubCategoryForm from 'utility/SubCategoryForm';
...
const SubCategory = ({ match }) => {
  ...
  const onSubmit = async (values , {resetForm}) => {
    const value = values;
    value.parentId = values.parentId?.value;
    setLoading(true);
    if(operations === 'A'){
    await postData(value)
    NotificationManager.success('New Sub Category has been added!','Success!')
    }else{
      const {id} = selectedItem;
      await putData(id,value)
    NotificationManager.success('Sub Category has been updated successfuly!','Success!')
    }
    get()
    toggleModal();
   ...
   const deleteProduct = async() => {
    ...
    NotificationManager.success('Sub Category has been deleted successfuly!','Success!')
    get();
    toggleAlert('');
   }
    ...
    <>
                  ...
                  <table className=" table ">
                    <thead>
                      <tr>
                        ...
                        <th className={`sort-column ${tableOptions.sort === 'name' ? sort[tableOptions.direction] :'' }`} onClick={()=>sorting('name')}>Sub Category Name</th>
                        <th className={`sort-column ${tableOptions.sort === 'parent.name' ? sort[tableOptions.direction] :'' }`} onClick={()=>sorting('parent.name')}> Category Name</th>
                        ...
                        data.map((item, index) => (
                          <tr key={item.name}>
                            ...
                            <td>
                              <img
                                className="table-img"
                                src={`${IMAGE_URL}/sub-category/${item.image}`}
                                alt={item.name}
                              />
                            </td>
                            <td>{item.name}</td>
                            ...
                            <td className="float-right">
                              <Row>
                                ...
                                <button  type="button" tabIndex="0" onClick={()=>editAndView('E' , item)} className="glyph-icon action-icons m-2 simple-icon-pencil" />
                                <button type='button' tabIndex="0" onClick={()=>editAndView('V' , item)} className="glyph-icon m-2 action-icons simple-icon-eye" />
                              </Row>
                            </td>
                          </tr>
                        ))}
      ...
      >
        {({ errors , setFieldValue ,values ,touched,setFieldTouched}) => (
          ...
          >
            <SubCategoryForm setFieldValue ={setFieldValue} setFieldTouched={setFieldTouched} operation={operations}  name='image' errors={errors}  values = {values } touched={touched} />
          </CustomModal>
        )}
      </Formik>
    </>
