...
import { getProductCategory, patchProductCategory, postProductCategory, putProductCategory } from 'services/productCategory';
import { NotificationManager } from 'components/common/react-notifications';
import { IMAGE_URL } from 'constants/defaultValues';
import CustomModal from 'utility/CustomModal';
import ProductCategoryForm from 'utility/ProductCategoryForm';
...
const title= {
  'A' : 'Add Product Category',
  'E' : 'Edit Product Category',
  'V' : 'View Product Category',
}
const sort = {
  1 : 'sort-asc',
  "-1" : 'sort-desc',
    ...
    <>
                        ...
                        data.map((item, index) => (
                          <tr key={item.name}>
                            ...
                            <td>
                              <img
                                className="table-img"
                                src={`${IMAGE_URL}/product-category/${item.image}`}
                                alt={item.name}
                              />
                            </td>
                            <td>{item.name}</td>
                            ...
                            <td className="float-right">
                              <Row>
                                ...
                                <button  type="button" tabIndex="0" onClick={()=>editAndViewCategory('E' , item)} className="glyph-icon action-icons m-2 simple-icon-pencil" />
                                <button type='button' tabIndex="0" onClick={()=>editAndViewCategory('V' , item)} className="glyph-icon m-2 action-icons simple-icon-eye" />
                              </Row>
                            </td>
                          </tr>
                        ))}
      ...
      >
        {({ errors , setFieldValue ,values ,touched}) => (
          ...
          >
            <ProductCategoryForm setFieldValue ={setFieldValue} operation={operations}  name='image' errors={errors}  values = {values } touched={touched} />
          </CustomModal>
        )}
      </Formik>
    </>
